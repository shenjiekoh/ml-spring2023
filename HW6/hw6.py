...
Original file is located at
    https://colab.research.google.com/drive/1ABjHiNxKWM8uKFAL9ZIqz6Qp5G9Zb2hZ
...
"""
Variance Scheduler Reference: https://huggingface.co/blog/annotated-diffusion
"""
...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_loop(self, shape, return_all_timesteps = False):
        ...
        sampled_img_idx = torch.randint(0, batch, (1,)).item()
        sampled_timesteps = torch.linspace(0, self.num_timesteps, 5)
        sampled_timesteps = [int(i) for i in sampled_timesteps.tolist()]
        fig, axes = plt.subplots(nrows=1, ncols=5, figsize=(20,4))
        imgs_tensor = torch.stack(imgs, dim=0).permute(1, 0, 3, 4, 2)  # T x B x C x H x W -> B x T x H x W x C
        sampled_img = self.unnormalize(imgs_tensor[sampled_img_idx]) # 1 x T x H x W x C
        for i, t in enumerate(sampled_timesteps):
            sampled_img_t = sampled_img[t].squeeze().cpu()
            sampled_img_t = torch.clamp(sampled_img_t, 0, 1)  # the value of sampled_img_t might out of range [0,1]
            axes[i].imshow(sampled_img_t.numpy())
            axes[i].axis('off')
            axes[i].set_title("timestep {}".format(t))
        plt.show()
...
!stylegan2_pytorch --data ./faces --name stylegan2 --results_dir ./results_stylegan2 --image-size 64 --batch-size 16 --num-train-steps 40000
...
outputs = 1000
noise   = torch.randn(outputs, 512).cuda() # noise
styles  = loader.noise_to_styles(noise, trunc_psi = 0.9)  # pass through mapping network
...
for i in range(10):
    ...
    for j in range(eval_batch_size):
        save_image(images[j], f'./submission_stylegan2/{i*eval_batch_size+j+1}.jpg') # save your images, or do whatever you desire
...
2. [Denoising Diffusion Pytorch](https://github.com/lucidrains/denoising-diffusion-pytorch)
3. [StyleGAN2 Pytorch](https://github.com/lucidrains/stylegan2-pytorch)
"""